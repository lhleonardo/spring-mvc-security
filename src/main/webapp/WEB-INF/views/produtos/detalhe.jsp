...
<tags:pageTemplate titulo="${produto.titulo } - Casa do Código">
	<article id="${produto.id }">
		...
		<section class="buy-options clearfix">
			<form:form action="${s:mvcUrl('CC#add').build() }" method="post" class="container">
				<input type="hidden" value="${produto.id }" name="produtoId" />
				<ul id="variants" class="clearfix">

					<c:forEach items="${produto.precos }" var="preco">
						<li class="buy-option"><label class="variant-label" for="product-variant-9720393823">${preco.tipo} <input
								type="radio" name="tipo" class="variant-radio" value="${preco.tipo}" checked="checked" />
						</label> <small class="compare-at-price">R$ 39,90</small>
							<p class="variant-price">${preco.valor }</p></li>
					</c:forEach>


				</ul>
				<button type="submit" class="submit-image icon-basket-alt" alt="Compre Agora '${produto.titulo }'"

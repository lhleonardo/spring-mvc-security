...
<tags:pageTemplate titulo="Livros de Java, SOA, Android, iPhone, Ruby on Rails e muito mais - Casa do Código">
	<section class="container middle">
		<a href="${s:mvcUrl('PC#lista').build()}">Voltar para os produtos</a>
			...
			<tbody>
				<c:forEach items="${carrinho.itens }" var="item">
					<tr>
						...
						<td class="quantity-input-cell"><input type="number" min="0" name="quantidade"
							value="${carrinho.getQuantidade(item) }" /></td>
						<td class="numeric-cell">${carrinho.getValorTotal(item) }</td>
						<td class="remove-item">
							<form:form action="${s:mvcUrl('CC#remove').arg(0, item.produto.id).arg(1, item.tipoPreco).build()}" method="post">
